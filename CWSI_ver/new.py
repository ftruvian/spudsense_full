...
def get_user_command() -> str:
    """Prompts the user for a valid soil sensor command."""
    while True:
        prompt = f"Enter soil sensor command ({', '.join(VALID_COMMANDS)}): "
...
def communicate_with_devices():
    ...
    sends pump trigger command only if stress condition is met, and finally
    sends the motor movement command.
    """
    # 1. Get the command from the user
    command = get_user_command()
    print(f"\n--- Initiating read for Sensor {command} ---")
    ...
    if moisture_data is None:
        return # Exit if moisture reading failed
    ...
    if temp_data is None:
        return # Exit if temp reading failed
    ...
    # The motor is signaled after all data and decisions have been made.
