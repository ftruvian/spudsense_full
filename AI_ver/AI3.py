...
LINEAR_SCALER_PATH = 'linear_regression_scaler.joblib'
...
def report_system_resources(start_time: float, end_time: float):
    """Calculates and prints the average system resource consumption using psutil."""
    # Check if psutil was successfully imported (globally)
    if 'psutil' not in globals():
        print("\nSystem resource report skipped (psutil not available).")
        return

    try:
        # psutil.cpu_percent(interval=None) returns the average CPU usage 
        # since the last non-blocking call, which we made before the main loop.
        avg_cpu = psutil.cpu_percent(interval=None)

        # Memory usage (virtual memory snapshot at the end)
        mem = psutil.virtual_memory()
        total_time = round(end_time - start_time, 2)
        
        # Convert bytes to GB for memory report
        ...
        print(f"Average CPU Usage during execution: {avg_cpu:.1f}%")
        print("--- Memory Usage (End of Script) ---")
        print(f"Total System Memory: {mem_total_gb:.2f} GB")
        print(f"Used Memory: {mem_used_gb:.2f} GB")
        print(f"Memory Usage Percent: {mem.percent:.1f}%")
        print("=======================================================")

    except Exception as e:
        print(f"\n[RESOURCE REPORT ERROR] Failed to generate report: {e}")
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    if cpu_monitor_ready:
        report_system_resources(start_time, end_time)
    else:
